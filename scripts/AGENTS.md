...
- `multi-surface-semantic-diff.sh` / `multi-surface-semantic-diff.py` 是标准 Markdown 多交付面语义一致性门禁；用北京测试样本验证 API direct/API job/Web direct/Web job/Bot dry-run canonical renderer 在八字与紫微报告上的 normalized semantic hash 一致，只保存 hash、长度和归一化策略，不保存报告正文；真实 Bot live 与 HF Space 仍是外部连通验证待执行。
