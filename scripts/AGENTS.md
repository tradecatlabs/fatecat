...
- `geo-audit.py` 对公开根跳转、Web canonical/Schema.org、robots、sitemap、llms、OpenAPI、capability 和 provider 做真实 HTTP 审计并输出量化 JSON；无法从公开 HTTP 得到的索引、引用、流量和转化指标必须保持外部待验证。
...
- `scripts/geo-audit.py -> public Web + llms.txt + robots.txt + sitemap.xml + OpenAPI + capability/provider registries`
