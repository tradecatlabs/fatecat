...
- `classics-dataset-clean.py` 是 canonical 命理典籍的确定性内部数据集构建器；严格校验 UTF-8、来源 hash 和版权状态，输出可追溯文档/段落/切片、重复标记和质量报告到 ignored local-state，不改写原文、不自动去重，也不授予训练、生产或公开分发权限。
...
- `scripts/classics-dataset-clean.py -> domains/fate-analysis/data-products/classics + contracts/fate/data-supply-chain/schemas/classics-clean-dataset.schema.json`
