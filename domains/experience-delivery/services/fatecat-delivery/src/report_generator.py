...
def _rule_depth_narrative(result: dict[str, Any], key: str) -> str:
    """读取后端规则摘要；报告层只呈现，不重新计算命理规则。"""
    depth = result.get(key, {})
    if not isinstance(depth, dict):
        return ""
    summary = depth.get("narrativeSummary", {})
    if not isinstance(summary, dict):
        return ""
    return str(summary.get("markdown", "") or "").strip()
...
def generate_bone_section(result: dict[str, Any]) -> str:
    ...
    if bw:
        ...
        if comp:
            ...
            day = comp.get("day", {})
            hour = comp.get("hour", {})
            lines.append(f"  - 年柱 {year.get('ganZhi', '')}：{year.get('weightCn', '')}")
            month_label = f"{month.get('monthCn', '')}月"
            if month.get("isLeapMonth"):
                source_month = month.get("sourceMonthCn", "")
                effective_month = month.get("effectiveMonthCn", "")
...
def generate_ziwei_interpretation_section(result: dict[str, Any]) -> str:
    ...
    narrative = _rule_depth_narrative(result, "ziweiRuleDepth")
    if narrative:
        lines.append(narrative)
        lines.append("")
