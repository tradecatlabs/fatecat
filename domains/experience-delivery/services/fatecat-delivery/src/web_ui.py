...
def _render_workbench_style() -> str:
    return "\n".join(
        [
            ...
            "#sidebar-toggle { position: absolute; inset-block-start: 0; inset-inline-start: 0; pointer-events: auto; appearance: none; border: 0; background: transparent; padding: 0; margin: 4px 0 0 4px; font: inherit; cursor: pointer; }",
            "#sidebar-toggle[data-glyph-measured] { inline-size: calc(var(--toggle-glyph-inline-ratio) * 1em); block-size: calc(var(--toggle-glyph-block-ratio) * 1em); line-height: calc(var(--toggle-glyph-block-ratio) * 1em); }",
            "#control-plane { position: absolute; inset-block: 0; inset-inline-start: 0; z-index: 2; width: clamp(320px, 28vw, 440px); min-width: 0; max-height: 100vh; padding-block-start: var(--content-top, 0px); padding-inline: 1rem; padding-block-end: 2rem; overflow-x: hidden; overflow-y: auto; background: Canvas; box-sizing: border-box; }",
            ...
            "#data-plane { position: relative; width: 100%; height: 100vh; min-height: 0; z-index: 1; overflow: auto; padding-block-start: var(--content-top, 0px); box-sizing: border-box; }",
...
def _render_copy_script() -> str:
    return "\n".join(
        [
            ...
            "  if (!workspace || !control || !toggle) { return; }",
            "  const CONTENT_GAP = 4;",
            "  const syncLayout = () => {",
            '    toggle.removeAttribute("data-glyph-measured");',
            '    toggle.style.removeProperty("--toggle-glyph-inline-ratio");',
            '    toggle.style.removeProperty("--toggle-glyph-block-ratio");',
            '    workspace.style.removeProperty("--content-top");',
            "    const range = document.createRange();",
            "    range.selectNodeContents(toggle);",
            "    const glyph = range.getBoundingClientRect();",
            "    const fontSize = Number.parseFloat(getComputedStyle(toggle).fontSize);",
            '    if (!Number.isFinite(fontSize) || fontSize <= 0 || glyph.width <= 0 || glyph.height <= 0) { throw new Error("native workbench toggle glyph measurement failed"); }',
            '    toggle.style.setProperty("--toggle-glyph-inline-ratio", String(glyph.width / fontSize));',
            '    toggle.style.setProperty("--toggle-glyph-block-ratio", String(glyph.height / fontSize));',
            '    toggle.dataset.glyphMeasured = "true";',
            "    const toggleBox = toggle.getBoundingClientRect();",
            "    const controlBox = control.getBoundingClientRect();",
            "    const contentTop = toggleBox.bottom - controlBox.top + CONTENT_GAP;",
            ...
            '    toggle.textContent = collapsed ? "➡️" : "⬅️";',
            "    syncLayout();",
            "  };",
            "  renderShell();",
