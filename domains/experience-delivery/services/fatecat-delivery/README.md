...
- `/web` 当前不存在布局授权例外；浏览器默认渲染即为预期呈现。工作台三层由语义 `header#top-layer`、`fieldset#control-plane`、`article#data-plane` 表达，并以 `data-layer` / `data-workbench-layer` 登记；映射见 `governance/decisions/adr/ADR-0002-native-workbench-semantic-layer-map.md`。
