...
- `/web` 当前只具有 GATE-0001 登记的结构性工作台例外；浏览器无 CSS 时仍保持完整可读。工作台由 `#top-layer` / `#control-plane` / `#data-plane` 表达，固定 z-index `3/2/1` 和 `clamp(320px, 28vw, 440px)` 控制面，`#report-content` 在固定数据面安全区居中，视觉美化仍被禁止；映射见 `governance/decisions/adr/ADR-0002-native-workbench-semantic-layer-map.md`。
