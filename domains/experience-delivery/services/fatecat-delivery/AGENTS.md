...
- `src/public_discovery.py` 只负责公开 canonical 基址、权威说明页、可见 FAQ、Schema.org 实体图、robots 与 sitemap；不得承载命理规则、动态营销文案或未经证实的能力声明。
...
- `GET /about` 由 `public_discovery.py` 以实时 capability 元数据生成可引用正文；`GET /llms.txt` 只读取仓库根 `llms.txt` 并以 `text/plain` 暴露公开机器说明；`GET /robots.txt` 和 `GET /sitemap.xml` 确定性生成。Web 顶部只保留人类需要的项目、输出、存储和风险摘要，详细端点与字段契约归 `/about` 和 `llms.txt`。
