...
- `scripts/classics-dataset-clean.py`：从 `classics/*.txt` 确定性生成 ignored `classics-clean-v3` 内部派生数据集；按 source-hash policy 分离目录、重建语义段落并在章节内切片，保证文档标题身份唯一，分类同书/同家族/跨家族重复关系并聚合人工复核入口，不改变 canonical 原文或版权状态。
