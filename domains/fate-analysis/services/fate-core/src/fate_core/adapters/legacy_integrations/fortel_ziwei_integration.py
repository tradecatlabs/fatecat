...
└── tools/reference-repos/github/iztro-main/               # iztro 核心算法源码快照
...
from fate_core.support.paths import FATE_RUNTIME_ROOT, IZTRO_DIR

IZTRO_PATH = Path(IZTRO_DIR)
IZTRO_BUILD_ROOT = Path(FATE_RUNTIME_ROOT) / "vendor-build"
...
def _iztro_build_dir(source_dir: Path) -> Path:
    """根据源码路径生成隔离构建目录，避免不同测试/仓库互相污染。"""
    source_key = str(Path(source_dir).resolve())
    digest = sha256(source_key.encode("utf-8")).hexdigest()[:12]
    return IZTRO_BUILD_ROOT / f"iztro-main-{digest}"


def _prepare_iztro_build_dir(source_dir: Path) -> Path:
    """复制 iztro 源码到运行态构建目录，避免在 vendor 快照中写入 node_modules。"""
    source_dir = Path(source_dir)
    build_dir = _iztro_build_dir(source_dir)
    if build_dir.exists():
        try:
            existing_entry = _resolve_node_package_entry(build_dir)
        except RuntimeError:
            existing_entry = build_dir / "lib" / "index.js"
        if (build_dir / "node_modules").exists() and existing_entry.exists():
            return build_dir
...
def _ensure_iztro_ready(repo_dir: Path) -> Path:
    """确保 iztro 已安装依赖并生成入口文件，构建产物只写入运行态目录。"""
    ...
    working_dir = _prepare_iztro_build_dir(repo_dir)
    node_modules_dir = working_dir / "node_modules"
    entry_path = _resolve_node_package_entry(working_dir)
    ...
    if not entry_path.exists() and _has_npm_script(working_dir, "build"):
        _run_npm_command(working_dir, ["run", "build"], timeout=300)
        entry_path = _resolve_node_package_entry(working_dir)

    if not entry_path.exists():
        raise RuntimeError(f"iztro 入口缺失: {entry_path}；已尝试自动安装/构建，请检查 {working_dir / 'package.json'}")
