...
`domains/fate-analysis/services/fate-core/src/fate_core/providers/` 负责把纯命理输出拆成可组合的字段组装器；每个文件只关心一组字段，不直接承载 HTTP / Bot 交付。
...
- `base_chart.py`：本命盘基础字段；一次生成 canonical 地支关系，并把同一结果交给旧字段投影，禁止重复计算。
