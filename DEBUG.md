...
称骨模块曾使用二进制浮点数累计“钱”并截断小数，最高组合 `7.1` 被格式化为 `7两0钱`；首轮修复又只为七两一钱补了男女歌诀，其余可达重量仍错误回退到同一摘要，Markdown 继续输出 `3.7两` 等阿拉伯数字格式。
...
- 第二轮修复仍只覆盖 `7两1钱`，`3两7钱` 等其余 50 个可达重量继续返回通用摘要；这是假性“男女区分”。
- 男女歌诀全量接入时曾直接覆盖原 `text` 评语字段，导致“财禄厚重白手成家之命”等通用评语从报告中消失。
- 算准网男女表对照显示：男命歌诀覆盖二两一钱至七两二钱，女命歌诀覆盖二两一钱至七两一钱；两套歌诀范围不能强制对齐。
- 称骨报告同时输出“数值：3.7两”和“中文：3两7钱”，四项构成仍使用 `1.3两`、`0.5两` 等小数，不符合中文民俗文本的呈现契约。
...
3. 只在七两一钱区分男女即可代表完整男女版本。
   - Conflicts: 算准网和固定版本的现代实现都分别列出完整男命、女命歌诀；男命还独有七两二钱条目。

### Root Cause

模块没有把“整数钱计算表”“男女歌诀版本”“歌诀表范围”“闰月折算策略”和“中文展示文本”建模为不同边界，导致浮点表示、宽松字典回退、男女覆盖不完整、版本范围被强制对齐和展示格式混用共同污染结果。

### Fix

- 以整数“钱”作为唯一计算单位，中文重量使用 `divmod()` 格式化。
- 非法年柱、农历月日、时支和性别值直接报错，不再生成半残结果。
- 将七两二钱从当前权重组合的可执行范围中移出；男命歌诀表仍保留该条目并显式标记为不可计算，女命歌诀表不包含该条目。
- 保留有符号闰月，默认采用 `split_at_15`，并保留 `same_month` 作为显式版本复现策略。
- 男命和女命使用独立歌诀表：男命二两一钱至七两二钱，女命二两一钱至七两一钱；来源版本固定，并明确其不是历史权威。
- 年月日时权重继续以整数“钱”求和，当前全部组合最高为七两一钱；男命七两二钱保留在歌诀表中并标记为当前权重组合不可达，女命表没有该条目。
- Markdown 只展示中文重量：总重使用“三两七钱”，构成使用“一两三钱、五钱、九钱、一两”，不再输出小数重量。
- 通用评语与男女歌诀分字段保存：`summary` 保留原称骨评语，`text` 保存对应性别歌诀，Markdown 同时输出两者。
...
- 称骨与报告专项最终结果：`29 passed`。
- 称骨、报告、规则深度和策略定向回归：`101 passed`。
- 排除无关未跟踪任务测试后的仓库全量回归：`679 passed, 1 skipped`。
