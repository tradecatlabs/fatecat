...
- `runtime-backends.json`：登记 CalculationJob durable runtime 后端候选，当前 memory/sqlite 是本地 baseline，Postgres 已有 live smoke baseline 但仍是 planned external backend 候选，Temporal 是 future workflow orchestrator，Redis queue 只允许作为辅助队列。
...
- `RuntimeBackend` 的本地 gate 可以通过，但 `backend.postgres` 即使已有 live smoke baseline，也必须在生产多副本 worker、外部 Vault/KMS 和公网 webhook 证据完成前保持 `status=planned`，不能声明 external backend 已生产。
