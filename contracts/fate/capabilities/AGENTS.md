...
- `registry.json`：统一能力注册表；`availability` 是执行准入真相源，`maturity` 单独表达成熟度，`bazi` 是唯一默认报告能力。
...
- `profiles/`：各能力独立报告结构；除 `bazi` 外全部 `markdownDefault=false`；`bazi` 与 `ziwei` 还声明公开 Markdown 允许结构和机器专用字段边界。
