...
- `external-evidence-submission-readiness-audit.json`：定义外部证据提交准备度审计契约，聚合 proof-ref、live proof、operator packet、核心质量人审、第三方审计预演和 certification 产物，输出可提交/待提交/阻断矩阵；不执行真实 live、不提交证据、不解除 100% 阻断。
...
- external evidence submission readiness audit 位于 `scripts/external-evidence-submission-readiness-audit.py`，消费外部验证链路、人审 gate、第三方审计预演和 certification，检查 bundle schema、artifact hash 字段和 operator command hash 的提交准备度；它不执行真实 live、不保存敏感值、不把 pending 改成 passed。
