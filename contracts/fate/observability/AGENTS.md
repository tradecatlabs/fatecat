...
├── AGENTS.md
├── alert-rules.json
├── otel-backend-slo-evidence-contract.json
...
- `otel-backend-slo-evidence-contract.json`：OTel backend/SLO staged evidence 契约；只登记外部 collector/runtime/backend/SLO/alert proof refs 的脱敏 evidence schema 和反伪造负例。
...
- `scripts/otel-backend-slo-gate.sh`：OTel backend/SLO staged evidence gate；校验外部 collector runtime、trace backend、metrics backend、SLO dashboard、alert route、error budget 和 incident drill proof refs 的脱敏证据口径与反伪造负例。
- 这里不保存真实日志、真实请求体、真实 token、生产指标快照或 trace 数据。
- 当前 trace/SLO/alert、collector config 和 backend/SLO staged gate 都是本地 baseline；OpenTelemetry SDK exporter、真实 collector runtime、trace backend、Prometheus/Alertmanager、生产监控平台、真实流量 error budget 和 incident drill 仍是外部连通验证待执行。
