...
def test_runtime_backend_gate_summary(tmp_path):
    ...
    assert any("public webhook live smoke gate baseline" in item for item in stored["limits"])
...
def test_runtime_backend_contract_keeps_external_backends_planned():
    ...
    assert backends["backend.postgres"]["implementationStatus"] == "public_webhook_live_smoke_gate_baseline"
