...
def test_runtime_backend_gate_summary(tmp_path):
    ...
    assert any("worker heartbeat/polling smoke baseline" in item for item in stored["limits"])
...
def test_runtime_backend_contract_keeps_external_backends_planned():
    ...
    assert backends["backend.postgres"]["implementationStatus"] == "worker_heartbeat_polling_smoke_baseline"
    ...
    assert (
        "bash scripts/postgres-external-worker-restart-smoke.sh --allow-missing"
        in backends["backend.postgres"]["localVerification"]
    )
    assert (
    ...
    assert (
        "bash scripts/postgres-external-worker-restart-smoke.sh" in backends["backend.postgres"]["externalVerification"]
    )
    assert (
        "bash scripts/postgres-worker-heartbeat-polling-smoke.sh"
...
def test_runtime_backend_schema_and_resource_model_are_linked():
    ...
    assert "external_worker_restart_smoke_baseline" in runtime_schema["allowedImplementationStatus"]
    assert "job_worker_lease_smoke_baseline" in runtime_schema["allowedImplementationStatus"]
