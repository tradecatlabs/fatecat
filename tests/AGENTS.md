...
- `regression/test_classics_dataset_clean.py`：典籍内部派生数据集离线合同；覆盖确定性构建、物理换行语义重建、精确源行回放、目录分流、heading 内 passage、source-hash policy、书目复核边界、排除血缘、版权边界、重复只标记不删除、非法编码/控制字符拒绝、路径隔离和输出篡改检测，不读取私有 raw 或公网资源。
