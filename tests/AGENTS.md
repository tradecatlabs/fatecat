...
- `regression/test_classics_dataset_clean.py`：典籍内部派生数据集离线合同；覆盖确定性构建、无损规范化、source-hash policy、正文/envelope 分离、书目复核边界、排除血缘、来源追溯、版权边界、重复只标记不删除、非法编码/控制字符拒绝、路径隔离和输出篡改检测，不读取私有 raw 或公网资源。
