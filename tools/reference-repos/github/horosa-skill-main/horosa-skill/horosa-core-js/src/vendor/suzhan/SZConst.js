...
// headless 守卫：SZConst 在模块加载时读 localStorage 取「盘形/起宫制」偏好。skill 的 JS 恒在 headless
// node 跑（无浏览器持久化），且 node 实验性全局 localStorage 无 --localstorage-file 会抛 → 一律用空
// shim 回退默认（方盘 / 八字起宫）。仅影响 UI 偏好，不影响政余格局闭包用到的纯数据表。
