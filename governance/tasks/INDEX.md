...
| 0108 | measurement-infrastructure-current-release-artifact-proof | In Progress | P0 | 执行 0107 后续 P0 切片：为最终 main HEAD 生成当前 release artifact proof，触发 GitHub Actions Container workflow 的 push_image=true 以发布 GHCR image、上传 release artifacts、生成并验证 GitHub artifact attestation；同时生成本地 dry-run rollback drill evidence，并用 current-release-proof 聚合远端 Acceptance、Container release run、registry digest、attestation、release artifacts 与 rollback evidence。不得把 dry-run rollback 写成真实生产回滚，不得声明生产 API/HF/Bot/OIDC/SIEM/OTel/Vault/KMS live 完成。 | 0108-measurement-infrastructure-current-release-artifact-proof/ |
