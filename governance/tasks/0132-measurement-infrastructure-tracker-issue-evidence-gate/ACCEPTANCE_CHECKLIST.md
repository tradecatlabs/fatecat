...
Verify: roadmap 6.26 and existing tracker import package script reviewed.
...
Verify: contract, Python gate and shell wrapper exist.

Gate: gate does not execute external live calls, does not create issues, does not execute `gh`, and rejects sensitive/raw URL output.
...
Verify: local-ci has a tracker issue evidence gate step and focused pytest includes the tracker issue evidence gate test.
...
Verify: scripts/audit/tests AGENTS, roadmap and task index mention the tracker issue evidence gate.
...
| TP-02 | [x] contract added; [x] gate added; [x] wrapper added; [x] kind validation added; [x] sensitive/raw URL rejection added |
...
- [x] Gate validates input `kind`.
- [x] Gate validates package/evidence source binding.
- [x] Raw URL, sensitive assignment and placeholder markers are rejected.
