...
| `postgres` | `FATE_REPORT_JOB_STORE=postgres`，`FATE_REPORT_JOB_DATABASE_URL=<secret-env>`，`FATE_REPORT_JOB_POSTGRES_LIVE_EVIDENCE=<live-smoke.json>` | Postgres ReportJobStore external worker restart smoke baseline；使用 tracked DDL、job/event/outbox/config 表、job execution lease 和 webhook outbox conditional claim/release SQL。缺少 `psycopg` 或 DSN 时启动 fail-fast，不会静默 fallback。当前 live smoke、outbox worker lease negative smoke、job worker lease primitive smoke 与 external worker restart smoke 只证明真实数据库 schema/job/outbox/config 路径、outbox duplicate claim 负例、job execution worker lease primitive、过期 lease stale running job 可被重启 manager 恢复执行且双 manager 只有一个执行；不证明 production ready、exactly-once、公网 webhook live、外部 Vault/KMS 或 heartbeat/polling worker。 |
...
| gate | `bash scripts/runtime-backend-gate.sh --output-json <path>` | 本地校验 contract：`backend.postgres` 已有 public webhook live smoke gate baseline 但仍只能是 planned external candidate，`backend.sqlite` 只能 single-replica，`backend.redis_queue` 不能作为 source of truth。 |
...
| 边界 | 事件历史只证明任务生命周期可审计；本地 webhook retry/outbox trail、SQLite persistent outbox record baseline、restart-safe failure smoke、replayable recovery smoke、SQLite outbox redelivery baseline、SQLite encrypted webhook delivery config vault baseline、SQLite outbox lease claim/release baseline、Postgres outbox worker lease negative smoke、Postgres job worker lease primitive smoke 与 Postgres external worker restart smoke 已有；真实公网 webhook live smoke、外部 Vault/KMS、生产密钥生命周期、heartbeat/polling worker、长期多副本运行和 exactly-once 仍未完成。 |
...
当前 webhook 是本地可验证 callback baseline，已包含有限 retry、事件轨迹、SQLite persistent outbox record baseline、SQLite manager 重建后的 resolver redelivery baseline、本地 encrypted config vault baseline、SQLite outbox lease claim/release baseline、Postgres outbox worker lease negative smoke、Postgres job worker lease primitive smoke 和 Postgres external worker restart smoke；outbox 摘要不包含完整 webhook URL、webhook secret、lease owner、报告正文、姓名、出生地区或请求体。它仍不包含公网 live callback、接收端 SLA、外部 Vault/KMS、生产密钥生命周期、heartbeat/polling worker、长期多副本运行或 exactly-once，这些仍是外部连通验证待执行。
