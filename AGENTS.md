...
- 允许的例外仅限 `GATE-0001` 白名单：工作台入口 `<style>`、结构性 `#` 选择器、z-index `3/2/1`、`clamp(320px, 28vw, 440px)` 控制面、系统 `Canvas` 背景、报告内容居中/避让控制面、原生 Emoji 开关和必要 `@media` 宽度适配；禁止颜色、圆角、阴影、动画、品牌字体、视觉 class、卡片和装饰。
