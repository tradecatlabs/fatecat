...
- 允许的例外仅限 `GATE-0001` 白名单：工作台入口 `<style>`、结构性 `#` 选择器、z-index `3/2/1`、`clamp(320px, 28vw, 440px)` 控制面、系统 `Canvas` 背景、按钮继承字号下的 Canvas alpha 可见 Ink 裁剪、`--workbench-edge-gap: 4px` 驱动按钮上/左外边缘与控制面、数据面语义内容左右边缘、按钮底部到内容的 `4px` 间隙、原生常驻滚动槽、控制面内部长行与原生控件局部溢出、原生 Emoji 开关和 `max-width: 959px` 窄屏适配；禁止固定安全区、固定命中区、颜色、圆角、阴影、动画、品牌字体、视觉 class、卡片和装饰。
